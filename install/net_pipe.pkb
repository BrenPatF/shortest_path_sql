...
FUNCTION All_Nets 
  ...
  g_ts_id             PLS_INTEGER := Timer_Set.Construct('All_Nets');
...
--    Timer_Set.Init_Time(g_ts_id);

    l_next_nodes_lis := next_Nodes(p_node_id       => p_node_id,
                                   p_link_id_prior => p_link_id);
--    Timer_Set.Increment_Time(g_ts_id, 'next_Nodes: ' || p_root_node_id);
--    Timer_Set.Increment_Time(g_ts_id, 'next_Nodes');
